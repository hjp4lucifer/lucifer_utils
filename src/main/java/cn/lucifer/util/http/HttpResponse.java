...
public class HttpResponse {
	...
	public void readResponse(InputStream in) throws IOException {
		...
		if (StringUtils.isEmpty(statusLine)) {
			throw new IOException("get statusLine error: null statusLine");
		}
		// System.out.println("statusLine:" + statusLine);
		int pos = statusLine.indexOf(' ');
		if (pos < 0) {
			throw new IOException("get statusLine error:" + statusLine);
		}
		...
		if (null != charset) {
			if (charset.matches(".+;charset=.+")) {
				charset = StringUtils.split(StringUtils.split(charset, ';')[1],
						'=')[1];
			} else {
	...
	private static Map<String, String> readHeaders(InputStream in)
		...
		while (StringUtils.isNotEmpty(line = readLine(in))) {
